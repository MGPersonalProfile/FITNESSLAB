...
const RESULT_MSG: Record<RequestResult, string> = t.social.requestResult;
...
export default function Social({ userId }: { userId: string }) {
  ...
  const unfriend = async (fid: string) => {
    if (!window.confirm(t.social.confirmRemove)) return;
      ...
      <header className="px-5 pt-6">
        <div className="flex items-baseline justify-between">
          ...
          <span className="font-mono text-[10px] tracking-[0.3em] text-[var(--fg-dim)]">
            {t.social.alliesCount(friends.length)}
          </span>
        </div>
        <h1 className="font-display text-3xl tracking-[0.05em] text-[var(--fg)] mt-1 leading-none">{t.social.title}</h1>
